    ...
    <div className='bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden max-w-4xl mx-auto '>
      ...
      <form onSubmit={handleUpdateProfile} >
        <div className='p-6 space-y-6'>
          <div>
            <label className='block text-sm font-medium text-slate-700 mb-2'>Email Adsress</label>
          ...
          <InputField label="Full Name" name='name' icon={User} type='name' value={formData.name} onChange={handleInputChange} placeholder="Enter your full name" />

          <div className='pt-6 border-t border-slate-200 '>
            <h4 className='text-lg font-medium text-slate-900'>Business Name</h4>
        ...
        <div className='px-6 py-4 bg-slate-50 border-t border-slate-200 flex justify-end'>
          <button type='submit' disabled={isUpdating} className='inline-flex items-center justify-center px-4 py-2 h-10 bg-blue-900 hover:bg-blue-800 text-white font-medium text-sm rounded-lg transition-colors duration-200 focus:Outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:oppacity-50 disabled:cursor-not-allowed'>
            {isUpdating ? <Loader2 className='w-5 h-5 mr-2 animation-spin' /> : null}
